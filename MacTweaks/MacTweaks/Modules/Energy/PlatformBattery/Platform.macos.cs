...
namespace MacTweaks.Modules.Energy.PlatformBattery
{
    ...
    internal static class CVDisplayLinkInterop
    {
        ...
        public static double GetRefreshRate(uint display)
        {
            ...
            using var dl = Runtime.GetINativeObject<CVDisplayLink>(handle, false);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private delegate void IOPowerSourceCallback(IntPtr context);

        //Prevent GC from collecting delegate.
        private static IOPowerSourceCallback IOPowerSourceCallbackHandle;
        
        internal static CFRunLoopSource CreatePowerSourceNotification(Action callback)
        {
            var sourceRef = IOPSNotificationCreateRunLoopSource(IOPowerSourceCallbackHandle = new IOPowerSourceCallback(_ => callback()), IntPtr.Zero);

            if (sourceRef == default)
                return null;
            
            return Runtime.GetINativeObject<CFRunLoopSource>(sourceRef, true);
        }
    }
}
